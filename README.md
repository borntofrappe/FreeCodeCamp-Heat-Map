Link to the working pen right [here](https://codepen.io/borntofrappe/full/qKeGLM).
...
From what I can gather after a thorough revision of the codebase, the mistake may specifically be generated by the way I defined the y-axis, specifically by the way I included the scale for the vertical axis.

By revising the way the `scaleBand` is defined, in terms of range, domains and values included through it, the error was fixed in light of the following considerations:

- `scaleBand` is an _ordinal_ scale. This scale includes an array of values in its domain, while maintaining the range in the same format described for linear or time scales;

- in the project at hand, the scale plots the discrete values describing the name of the months, `January`, `February` and so forth, in the vertical space from `legendValues.size * 2` to `height`. The range _does not_ start at 0 as this would inevitably have the heat map positioned on top of the legend, which includes rectangle elements with the exact size of `legendValues.size`;

- given the domain, the values included for each rectangle elements need to be discrete, string values for the full name of the month. This requires the definition of a formatting function, which takes as argument an integer (which is what the data provides for the month) and turns it into a string;

  ```JS
  const formatTimeMonth = d3 
                          .timeFormat("%B");

  // after the data is included, and for the vertical coordinate of the rectangle elements
  .attr("y", (d) =>  yScale(formatTimeMonth(d["month"])))
  ```

- given the range, the height of the different rectangle elements is a twelfth of the range itself. This measure needs to account for the height, but also the starting point of the interval. As previously stated, this point is different than 0.

  ```JS
  // after the data is included, and for the height of the rectangle elements
  .attr("height", (height - legendValues.size*2)/ 12)
  ```

While the JavaScript code make sense, in light of the multiple line of comments included in it, there is much to learn regarding the library. On the top of my head, the way data is parsed, formatted, and the way scales are defined is still a topic primed for improvement.